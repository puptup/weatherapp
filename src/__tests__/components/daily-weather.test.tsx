...
describe("daily-weather", () => {
  ...
  it("should render list", () => {
    jest.spyOn(selectors, "dailyWeatherStateSelector").mockReturnValue(State.normal);
    jest.spyOn(selectors, "dailyWeatherSelector").mockReturnValue(mockWeatherList);
    jest.spyOn(selectors, "currentDaySelector").mockReturnValue(new Date(mockCurrDay));
    const { getAllByTestId } = renderWithProviders(<DailyWeather />);
    expect(getAllByTestId("daily-weather-item").length).toBe(3);
  });
});
